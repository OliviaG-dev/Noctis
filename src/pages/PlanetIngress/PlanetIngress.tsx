...
const PlanetIngressPage: React.FC = () => {
  ...
  const [openCurrentKeys, setOpenCurrentKeys] = useState<Record<string, boolean>>({});
  ...
  const { current, upcoming, history } = useMemo(() => {
    ...
    const currentIngresses = withDates
      .filter(({ startDate, endDate }) => startDate.getTime() <= now.getTime() && endDate.getTime() >= now.getTime())
  ...
  const safeUpcomingPage = Math.min(upcomingPage, Math.max(totalUpcomingPages - 1, 0));
        ...
        <div className="events-list-page planet-ingress-page">
          {current.length > 0 && (
            <section className="planet-ingress-section planet-ingress-section-current">
              <h2 className="planet-ingress-section-title">En cours ({current.length})</h2>
                ...
                {current.map((ingress) => {
                  ...
                  return (
                    <div key={key} className="planet-ingress-current-accordion">
                      ...
                      >
                        <span>{ingress.planet} en {ingress.sign}</span>
                        <span className="planet-ingress-current-icon" aria-hidden="true">
                          {isOpen ? '▾' : '▸'}
                        </span>
                      </button>
                      <div className={`planet-ingress-current-content ${isOpen ? 'open' : ''}`}>
                        {isOpen && (
                          <EventCard
                            event={ingress}
                            type="planet_ingress"
          ...
          {upcoming.length > 0 && (
            <section className="planet-ingress-section planet-ingress-section-upcoming">
              <h2 className="planet-ingress-section-title">À venir ({upcoming.length})</h2>
              <div className="planet-ingress-section-content">
                <div className="planet-ingress-pagination-block">
                  {renderIngresses(paginatedUpcoming, 'upcoming')}
                  {totalUpcomingPages > 1 && (
                    <div className="planet-ingress-pagination-controls">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setUpcomingPage((prev) => Math.min(prev + 1, totalUpcomingPages - 1))
                        }
                      >
                        Suivant
                      </button>
          ...
          {history.length > 0 && (
            <section className="planet-ingress-section planet-ingress-section-history">
              ...
              >
                ...
                <span className="planet-ingress-history-icon" aria-hidden="true">
                  {showHistory ? '▾' : '▸'}
                </span>
              </button>
              <div className={`planet-ingress-history-content ${showHistory ? 'open' : ''}`}>
